...
/**
 * A base class for an &lt;authorize&gt; tag that is independent of the tag
 * rendering technology (JSP, Facelets). It treats tag attributes as simple
 * strings rather than strings that may contain expressions with the exception
 * of the "access" attribute, which is always expected to contain a Spring EL
 * expression.
 * <p>
 * Subclasses are expected to extract tag attribute values from the specific
 * rendering technology, evaluate them as expressions if necessary, and set the
 * String-based attributes of this class.
 *
...
public abstract class AbstractAuthorizeTag {
	...
	/**
	 * Make an authorization decision by considering all &lt;authorize&gt; tag
	 * attributes. The following are valid combinations of attributes:
	 * <ul>
	 * <li>access</li>
	 * <li>url, method</li>
	 * </ul>
	 * The above combinations are mutually exclusive and evaluated in the given
	 * order.
	 *
	 ...
	 * "Expression-Based Access Control" chapter in Spring Security for details on
	 ...
	 * True is returned if the user is allowed to access the given URL as defined.
	 *
	 * @return the result of the authorization decision
	 * @throws IOException
	 */
